...
int find_c;
double c_begin_p,c_end_p,c_step_p;
int find_g;
double g_begin_p,g_end_p,g_step_p;
int nr_fold;
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
    ...
    for(i=1;i<argc;i++)
    {
        ...
        switch(argv[i-1][1])
        {
            ...
            case 'C':
                find_c = 1;
                sscanf(argv[i], "%lf,%lf,%lf", &c_begin_p, &c_end_p, &c_step_p);
                break;
            case 'G':
                find_g = 1;
                sscanf(argv[i], "%lf,%lf,%lf", &g_begin_p, &g_end_p, &g_step_p);
                break;
            case 'w':
                ++param.nr_weight;
